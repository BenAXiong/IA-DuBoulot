...
3. if the conversation was just created from the subject launcher, the workbench can now consume a client bootstrap payload so the conversation route itself owns the pending first learner turn and first-turn send, while the launcher can already front-load upload and extraction before the prompt is handed off
...
8. uploads attachments through signed upload targets, confirms them, keeps the current file list in a minimal right-side rail, now accepts pasted clipboard images in both the homework quick-start and the live conversation composer, and now resolves attachment type from the filename extension when browser MIME metadata is missing or unreliable so ordinary PDFs do not fail late with the older generic invalid-fields error
9. lets the student remove an uploaded file directly from that rail, with a confirmation step before the server-owned delete, now opens uploaded images directly inside the right rail first, exposes a lightweight PDF details dropdown with extraction status, page count, the stored short source summary, and an optional stored page/section outline when available, and now also retries provider-failed attachment extraction once automatically when the conversation opens
10. keeps the right rail pinned under the student header as a true split pane instead of letting it stretch alongside the full transcript length, now lets desktop users resize that rail manually, and on desktop keeps the outer conversation route itself fixed to the viewport so only the transcript and rails own vertical scrolling
...
13. keeps the live composer pinned at the bottom of the conversation view, now shows a chevron jump-to-latest control above it when the transcript is scrolled upward, appends the learner's message optimistically with a lightweight pending banban placeholder so the prompt no longer looks stuck in the textarea during the round trip, no longer renders the provisional conversation title or last-activity chrome inside the live chat body, now keeps the textarea writable while attachment upload or extraction is in progress, disables only the send action during that upload window with a hover tooltip, replaces the old static send icon with a simple tiered loading circle that advances through `prepare -> upload -> extract`, removes the old bottom upload-progress status card, restores the learner draft without auto-send when the subject launcher already discovered an attachment-prep failure, and seeds new shell conversations with a neutral per-subject `Subject_###` placeholder until a later AI summary can replace it
...
16. renders a fourth collapsible `Summary` section in the right rail; it stays quiet during an active session, starts collapsed even after completion, and once the learner clicks `Homework done!` the workbench now applies the returned student summary immediately in that rail instead of waiting for a page reload
17. keeps the explicit completion control at the bottom of the right rail, now asks for confirmation before actually locking the session, and in non-production builds also exposes a dev-only regenerate-summary button so summary testing can reuse the same completion route without adding a second summary endpoint; that dev button now refreshes only the learner-facing student summary instead of re-triggering the whole parent, tutor, and memory fan-out
...
- the live workbench now reuses the same client-side attachment staging guardrails as the homework launcher, including current attachment count and byte-budget preflight from the already-uploaded files in the conversation, so obvious limit violations fail locally before a signed upload is attempted
...
- PDF details in the right rail are metadata-only for now: they read `page_count`, `extraction_status`, `metadata.source_summary`, and optional `metadata.source_outline` from the attachment record, without generating new previews or re-reading the file on ordinary chat turns
...
- the subject launcher can now reach the conversation route in a paused bootstrap state: when upload or extraction fails before the first prompt is sent, the conversation opens with the learner draft restored in the composer and an explicit launcher-side error instead of burning that first prompt automatically
...
- the live coach reply path does not directly inspect image pixels or PDF page renders; it only receives extracted attachment text plus the persisted conversation or workspace context, so any learner-facing claim about what was "read" from a file must stay grounded in that text context. As of 2026-04-28, the relevant source/attachment text caps are raised to `20,000` chars for common 5-10 page PDF troubleshooting, but this is still bounded text context rather than direct page-level or image-native reasoning.
...
- on desktop conversation routes, page-level overflow is now intentionally hidden; only the left rail, transcript body, and right rail are supposed to scroll
...
- upload-route validation responses already expose field-level localized errors, and the browser helper now prefers those field-level messages over the older generic wrapper string when a create or confirm call fails
...
- the summary contract is now intentionally lean on the learner side: whether the provider or the deterministic fallback produced it, the student-facing recap should focus on what was worked on, which skills still look fragile, and one concrete next step instead of surfacing transcript counts, file bookkeeping, or hidden workspace-state metadata
