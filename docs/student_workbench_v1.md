...
- PDF details in the right rail are metadata-only for now: they read `page_count`, `extraction_status`, and `metadata.source_summary` from the attachment record, without generating new previews or re-reading the file on ordinary chat turns
