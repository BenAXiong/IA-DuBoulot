...
- `/app` now defaults to `view=dashboard`, which shows one card per student left-rail learning section; homework start/resume remains under `view=homework` and subject links keep `view=homework&subject=...`
