...
- the homework block in the student rail now also restores a hover `+` entry point for creating a fresh subject via `/app?view=homework&newSubject=1`; new-subject creation should remain discoverable even after the learner already has existing subject filters
