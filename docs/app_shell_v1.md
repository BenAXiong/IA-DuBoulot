...
- `/app` now defaults to `view=dashboard`, which shows a wide Homework row with subject links plus a two-column grid for the other student left-rail learning sections; homework start/resume remains under `view=homework` and subject links keep `view=homework&subject=...`
