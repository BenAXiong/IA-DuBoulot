...
- on the live student conversation route, the shell header now uses the subject as the eyebrow and shows the conversation title only once it is no longer the neutral `Subject_###` bootstrap seed; until then the header stays on a single subject eyebrow line instead of echoing either the provisional placeholder or the subject again as a fake title
