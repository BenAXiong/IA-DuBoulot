...
- on the live student conversation route, the shell header now uses the subject as the eyebrow and shows the conversation title only once it is no longer the neutral `Subject_###` bootstrap seed; until then the header falls back to the subject-only conversation chrome instead of echoing the provisional placeholder
