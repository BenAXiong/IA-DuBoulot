...
- the auth route now uses a compact viewport-fit layout with a mini HUD-style header, a single centered auth card, a stable-width segmented `Sign in` or `New user` switch, and only minimal orientation copy inside the card; more supporting content should be added only if a real comprehension gap appears
