...
- the landing route content uses doubled desktop side padding relative to the earlier landing shell so the viewport edge-to-content gap is visibly wider on large screens, while mobile and tablet padding keep the previous public-shell rhythm. The landing-specific header is intentionally excluded from that wider body padding and keeps the normal near-full-width shell spacing.
...
- the auth route now uses a compact viewport-fit layout with a mini HUD-style header, a single centered auth card, and a stable-width segmented `Sign in` or `New user` switch as the first visible block; any explanatory copy above the toggle and any footer microcopy should stay absent unless real testing shows it is needed
- the onboarding route now uses the same mini HUD-style header as auth, opts out of the shared public footer, removes eyebrow/subtitle chrome, and keeps only the remaining profile-essential fields that sign-up does not yet own; role selection stays in account creation and the standalone onboarding route is now a transitional step rather than a destination to enrich further
...
- profile/settings surfaces display the signed-in auth email as read-only account identity, while profile fields remain limited to display name, UI language, AI help language, and student age band. On `/app/settings`, those profile fields render read-only until the user explicitly enters modify mode.
...
- the student shell header now uses a simple eyebrow-and-title pattern only when the second line adds useful orientation. Dashboard, Homework, and Settings suppress that second line because their page content or account surface already carries the visible context, while live conversation routes keep the title offset to line up with assistant reply text after the avatar column.
...
- the subject rail keeps subject filters and active-homework counters, and each subject chevron can reveal up to the five most recent conversation titles for that subject with `...` when more exist; only one subject subsection is expanded at a time. The counter is intentionally the number of active unfinished homework items, not the total number of past conversations.
...
- when the learner is already on `/app?view=homework`, Homework and subject links in the left rail are intercepted as client-side filter changes: the shell updates the highlighted subject optimistically, writes the new `/app?view=homework&subject=...` URL with `window.history.replaceState`, and notifies the Homework launcher through a local browser event. From Dashboard, settings, or live conversation routes, those same links remain normal Next links so real navigation still works.
- the root Homework page keeps all subject shortcuts unselected on entry and lists homework items across all subjects by latest activity. A compact vertical-ellipsis shortcut editor controls browser-local default-subject visibility, drag order, and custom subject labels without changing canonical conversation subjects.
