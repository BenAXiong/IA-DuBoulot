...
- on the live student conversation route, the shell header now uses the subject as the eyebrow and shows the current conversation title on the second line; if AI summarization has not replaced the neutral `Subject_###` bootstrap seed yet, the header now shows that placeholder instead of leaving the second line blank or repeating the subject as fake title copy
