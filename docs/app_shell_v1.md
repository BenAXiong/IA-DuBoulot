...
- on the live student conversation route, the shell header continues to show the subject rather than the conversation title; the workbench no longer renders the provisional stored title in the conversation body, and summarized titles should only surface later through shell chrome once the app can positively tell they are polished AI summaries rather than the neutral `Subject_###` bootstrap seed
