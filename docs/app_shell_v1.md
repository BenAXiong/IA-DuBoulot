...
- the landing-specific header uses that Student, Parent, or Tutor identity selector as its center control instead of generic Product/Features/Pricing anchors. The pricing page remains reachable from a compact `?` button beside the theme toggle.
- the hero's current `banban workspace` panel is a placeholder product-preview mock in `components/landing/public-landing-page.tsx`, not a real workspace embed; replace it with actual product media once captures are ready.
- identity-specific landing feature sections now share the same default structure: section title, short description, a large GIF/demo frame taking roughly two-thirds of the desktop row, and three adjacent feature cards taking the remaining third. Parent and Tutor use that template now; future Student copy should use the same section shape unless the landing brief changes.
