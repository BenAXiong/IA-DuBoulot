...
Status note: `P5.1` and `P5.2` are complete as bounded governance tasks. The audit above remains the prioritized debt map, and the first implemented slices deepen two interfaces without a sweeping rewrite: `scripts/smoke-app-harness.mjs` now owns local Next startup, cookie/auth handling, request IDs, and response helpers for five scenario adapters; `lib/oversight/summary-selection.ts` now owns the language-preference/fallback rule that the parent service consumes and the localization verifier tests. The large conversation, invitation, memory, copy, and workbench modules remain explicit future slices rather than hidden debt.
