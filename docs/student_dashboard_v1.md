...
- when the learner already has subjects, the left homework rail now restores a hover `+` affordance that reopens that same launcher on `/app?view=homework&newSubject=1`, so creating a new subject no longer depends on the no-history empty state
