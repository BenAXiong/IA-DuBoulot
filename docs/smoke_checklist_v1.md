...
- 2026-04-04 local `npm run smoke:student-flow`: failed on the existing sample-PDF extraction assertion after the student UI pass; the API flow still created the draft and completed provider extraction, but the harness rejected a shorter-than-expected extracted-text block (`[Source : fractions-partage.pdf]` plus one line of body text). This looks like smoke-harness or corpus drift rather than a UI regression, but it must be reviewed before the next full demo-regression run.
