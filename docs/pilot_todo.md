...
Status note: the student chat now ships a first learner-visible mode switch for `fast`, `thinking`, and `interactive`, but the implementation still follows the same Pilot-safe principle: these are prompt-level coaching variants on the same model path, not separate underlying AI models. `P2.4` remains open because the team still needs to evaluate transcript quality by subject family and decide whether those modes should stay learner-visible or retreat back into operator-facing experimentation.
