...
- Subject-doc pilot upload caps are separate from chat-only attachment caps: provider-extracted PDF/DOC/DOCX resources allow up to `20 MB` per file, direct TXT/MD/JSON resources allow up to `5 MB`, and ordinary chat-only PDFs remain at the existing chat-attachment cap. There is no dedicated subject-library total byte cap yet; upload actions still count against the general usage quota.
...
The seventh slice fixed the first larger-workbook upload failure, then the cap was tightened again for Pilot cost control:
...
- keep provider-extracted subject-resource caps at `20 MB` per file for PDFs and Word documents during Pilot measurement
