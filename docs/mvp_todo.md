...
Status note: the shared theme bootstrap now follows the operating-system light or dark preference by default and clears older local theme overrides, while the visible public and authenticated MVP chrome no longer exposes theme controls. The `/app` shell also drops the pilot badge, the student start card no longer prints a literal route hint, and the admin overview text now reads like a real trust or audit surface instead of scaffolding.
