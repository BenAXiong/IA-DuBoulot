...
Status note: `/auth` now also opts out of the shared public footer and uses a viewport-fit layout with a desktop-only information rail, a centered `Sign in` or `New user` switch, and lighter copy density on smaller screens so the shared auth entry no longer depends on page scroll to expose its full form.
