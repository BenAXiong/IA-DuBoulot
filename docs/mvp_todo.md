...
Status note: the public landing is now a story-first product entry instead of a toolbox-like shortcut page, with the old quick links and pilot context grouped into a floating helper button above the Vercel helper area, the public language selector moved into a globe-menu dropdown, the top bar widened and simplified to wordmark plus controls, the hero and closing CTA centered on a wider page canvas, three alternating `preview + 3 glass cards` product rows with matched GIF height, neutral moving placeholder GIFs now stored in `public/landing/`, a slow shared CTA gradient drift, and the floater also exposing previewable `light`, `dark`, `smooth`, `warm`, and saved `custom` themes.
