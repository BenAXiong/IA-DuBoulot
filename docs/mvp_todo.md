...
Status note: `/auth` now also opts out of the shared public footer and uses a viewport-fit layout with a compact HUD-style header, a desktop-only information rail, a centered `Sign in` or `New user` switch with stable width, lighter product-facing copy, and smaller cards that no longer fill the full viewport height.
