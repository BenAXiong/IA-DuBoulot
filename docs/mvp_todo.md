...
Status note: `/auth` now also opts out of the shared public footer and uses a viewport-fit layout with a compact HUD-style header, a single centered auth card, a stable-width `Sign in` or `New user` switch, lighter product-facing copy, and only minimal orientation chips inside the card instead of a separate left information rail.
