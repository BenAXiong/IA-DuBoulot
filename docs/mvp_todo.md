...
Status note: the public landing is now a story-first product entry instead of a toolbox-like shortcut page, with the old quick links and pilot context grouped into a floating helper button above the Vercel helper area, the shared theme control simplified to a single icon toggle, the public language selector moved into a globe-menu dropdown, and the home route now owning its own bottom CTA instead of reusing the heavier public footer.
