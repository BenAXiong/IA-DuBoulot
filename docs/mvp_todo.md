...
Status note: the public landing is now a real product-facing entry instead of a toolbox-like shortcut page, with the old quick links grouped into a floating helper button above the Vercel helper area and the shared theme control simplified to a single icon toggle.
