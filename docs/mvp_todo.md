...
Status note: the student role now also uses a dedicated chat-first shell with a collapsible subject rail, a minimal top bar, a placeholder avatar dock, a direct subject quick-start that creates a bare conversation shell before opening the live chat, and a flatter conversation layout with a lighter sources/completion rail; `/app/new` remains only as a quieter richer-intake fallback while the remaining decisions about true subject entities, more implicit conversation creation, optional completion, and real avatar media stay deferred to Pilot work.
