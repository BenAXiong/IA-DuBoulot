...
- the public landing on `/` is now a real product-facing page instead of a shortcut toolbox, and the old direct quick links are grouped into a floating helper button so the public narrative stays calmer while operator access remains one click away.
