...
- the public landing on `/` is now a story-first product page instead of a shortcut toolbox, with quick links moved into the full-width navbar, a globe-menu language selector, and an identity-aware revamp in `components/landing/public-landing-page.tsx` that can later swap placeholder media and incomplete student or tutor copy without redoing the layout.
...
- the 2026-08-29 canonical regression is green through typecheck, lint, localization contracts, production build, and deployed public-language checks, but fixture-backed auth/RLS/product smokes are blocked before mutation because the Supabase hostname configured locally and in Vercel no longer resolves; `A0.2.1` is reopened until the hosted project is restored or replaced.
